    ...
    <b-col cols="10">
      ...
      <div class="workings">
        ...
        <div v-for="workHour in currentStaff.workinghours" :key="workHour.id"
          ...
          <template v-else>
            <div class="workingTime small">Day off</div>
          </template>
        </div>
      ...
      <div class="workingAction">
        <button class="saveBtn" @click="updateStaffWorkingHour({user: $router.history.current.params.id})">
          Save
        </button>
...
export default {
  ...
  methods: {
    ...mapActions({
      updateStaffWorkingHour: 'updateStaffWorkingHour'
    }),
    ...mapMutations({
